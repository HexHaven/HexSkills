...
metadata:
  hermes:
    ...
    related_skills: [hex-seed, hex-build]
---

# Nyx Recap

Produce a compact, evidence-backed recap or handoff of the current Nyx project/workflow (`hex-seed`, `hex-build`) so a new Hermes session can continue without the operator re-pasting the conversation. This skill only reports state — it never advances PLAN/APPLY/QUALIFY/UNIFY, never shapes a spec, and never mutates anything.

The canonical `hex-seed` → `hex-build` handoff is `.nyx/HANDOFF.md`, written by `hex-seed` itself (its contract is documented in the `hex-seed` skill, not here); `hex-recap` does not own or replace that mechanism and is never a required step between them. Use `hex-recap` for supplemental recovery: long sessions, interrupted work, manual/ad-hoc handoffs, or general context recovery — including cases with no repository or no `.nyx/` state at all.

## When to Use

- "recap", "summarize current state", "where are we"
- "handoff", "continue in new session", "compact context"
- An active `hex-seed` discovery or `hex-build` PLAN/APPLY/QUALIFY/UNIFY loop needs a checkpoint before a session ends.

Don't use for: running `hex-seed` discovery itself, executing `hex-build` stages, creating or auditing skills (`hex-skillsmith`), or general-purpose document summarization unrelated to a Nyx workflow.
...
1. **Identify scope** — determine whether an active `hex-seed` (IDEA/DISCOVER/SHAPE/SPECIFY/READINESS/HANDOFF) or `hex-build` (PLAN/APPLY/QUALIFY/UNIFY) workflow is in play from the current conversation. If neither, say so plainly and fall back to a generic session recap. Done when the active workflow (or its absence) is named.
...
6. **State workflow continuation** explicitly:
   - Active `hex-seed`: say whether the next step is *continue seed*, *request a consolidated `hex-build` handoff*, or *start `hex-build` PLAN*.
   - Active `hex-build`: name the current stage (PLAN/APPLY/QUALIFY/UNIFY), scope, changed files, verification status, divergence from plan, and closure state (`PASS`/`FAIL`/`BLOCKED`/`CANCELLED`/open).
   Done when the next concrete action is a single, unambiguous sentence.

## Safety Boundaries

- Read-only throughout — no commit, push, deploy, file edit, or remote mutation, even inside `hex-build` APPLY.
...
- Answering questions the user didn't ask by dragging in unrelated skills (`hex-seed`/`hex-build` themselves) instead of just reporting their state.

## Verification

- [ ] Active workflow (`hex-seed`, `hex-build`, or neither) is named with evidence.
